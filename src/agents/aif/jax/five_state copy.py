...
class JaxFiveStateAgent(JointWrapper):
    ...
    def __init__(
        ...
        self.agent = make_agent(policy_len=policy_len, lr_B=lr_B, alpha=alpha, gamma=gamma)
...
if __name__ == "__main__":
    ...
    agent = JaxFiveStateAgent(seed=0, lr_B=1.5, update_interval=100, alpha=1)
    agent2 = axl.DBS()
    agent3 = axl.APavlov2011()

    gtft = axl.ContriteTitForTat()
    match1 = Match((agent, gtft), turns=1000, noise=0.10)
    match1.play()
    print(match1.final_score())

    match2 = Match((agent2, gtft), turns=1000, noise=0.10)
    match2.play()
    print(match2.final_score())

    match3 = Match((agent3, gtft), turns=1000, noise=0.10)
    match3.play()
